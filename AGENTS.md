...
- `Scaffold`'s `contentWindowInsets` only applies to the body slot's `innerPadding` — it is NOT applied to the `bottomBar` slot. `NavigationBar`/`BottomAppBar` consume insets internally, but a custom composable passed as `bottomBar` (e.g. a plain `Button`) does not; under `enableEdgeToEdge()` with a 3-button (non-gesture) navigation bar, it gets rendered behind/clipped by the system nav bar. Add `.navigationBarsPadding()` directly to that composable, ordered before any content `.padding(...)` in the modifier chain (see `AlarmDetailScreen`'s `bottomButtonModifier`, matching `BottomMenuScreen`'s `FloatingBottomNavigationBar` handling).

- Watch alarms (`:watch-app`): alarms are scheduled locally with `AlarmManager.setAlarmClock` from the phone-synced `WatchAlarmSnapshot`, in `Asia/Seoul` (the UI labels alarm times as Korea time), with `days` as 0=Mon..6=Sun. `RescheduleStockAlarmsUseCase` is idempotent and is the only place that reconciles schedules; call it instead of scheduling ad hoc. The firing path in `StockAlarmReceiver` deliberately does three things in order - acquire an `ACQUIRE_CAUSES_WAKEUP` wake lock, post the `CATEGORY_ALARM` full-screen notification, then start `StockAlarmActivity` directly - because on Samsung watches the OEM notification layer relays the full-screen `PendingIntent` and the launch is blocked by BAL, leaving only a notification. Do not drop the wake lock or the direct start, and keep the notification as the fallback. Read `docs/watch-alarm-manager.md` before changing `service/StockAlarm*`, `data/alarm/`, or `domain/alarm/`, and update that doc in the same change.
