...
/**
 * [safeApiCall]과 동일하지만, 바디 없는 성공 응답(예: `204 No Content` DELETE)을 위한 변형.
 * `body != null` 검사를 하지 않으므로 204 응답도 정상적으로 [NetworkResult.Success]로 판정한다.
 */
suspend fun safeApiCallNoContent(execute: suspend () -> Response<Unit>): NetworkResult<Unit> =
    try {
        val response = execute()
