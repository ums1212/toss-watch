 ...
 * 3. 락 대기 중 다른 요청이 이미 토큰을 갱신해 뒀다면 그 토큰으로 즉시 재시도한다.
 * 4. 그 외에는 `POST /api/v1/toss-watch/auth/refresh/`를 suspend 호출해 새 토큰을 발급받는다.
...
class TokenAuthenticator @Inject constructor(
    ...
    // 동시 401 갱신을 직렬화하는 락. 스레드를 블로킹하는 synchronized 대신
    // 코루틴을 일시 중단시켜 OkHttp 워커 스레드 고갈을 방지한다.
        ...
        return runBlocking(Dispatchers.IO) {
            refreshMutex.withLock {
                ...
                if (currentAccessToken != null && currentAccessToken != failedAccessToken) {
                    return@withLock response.request.withBearerToken(currentAccessToken)
                }

                when (val result = refreshAccessToken(refreshToken)) {
    ...
    /**
     * `POST /api/v1/toss-watch/auth/refresh/`를 suspend 호출한다.
     * Retrofit이 내부적으로 최적화된 스레드에서 갱신 요청을 수행하며,
